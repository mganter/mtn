...
function RegisterStop(event)
  ...
  if not storage.MTL.surfaces[surface.index].stops[event.entity] then
    ...
    if not cc then
      lamp.destroy()
      train_stop.destroy()
      MTN_Log(LEVEL.ERROR, "could not create ConstantCombinator for train stop " .. train_stop.backer_name)
      return
    end
...
function ReadConfig(umbrella)
  ...
  if role_count == 0 then
    MTN_Log(LEVEL.INFO, "no role found for stop " .. umbrella.train_stop.backer_name)
    DeregisterStop(umbrella)
    return false
  end
  ...
  for stop_id, _ in pairs(storage.MTL.surfaces[surface.index][Roles.DEPOT]) do
    ...
    if umbrella.role == Roles.DEPOT then
      ...
      if #train.carriages < 2 then
        ...
        MTN_Log(LEVEL.ERROR, "train in stop \"" .. umbrella.train_stop.backer_name .. "\" has too few carriages")
        goto continue
      end
...
function GetOffers(surface)
  ...
  for stop_id, _ in pairs(storage.MTL[Roles.PROVIDER]) do
    ...
    for _, value in ipairs(signals) do
      ...
      if type == "item" or type == "fluid" then
...
function Tick()
  ...
  MTN_Log(LEVEL.TRACE, "all_requests: " .. dump(all_requests))
  MTN_Log(LEVEL.TRACE, "all_offers: " .. dump(all_offers))
  MTN_Log(LEVEL.TRACE, "current orders: " .. dump(storage.MTL.surfaces[surface.index].train_orders))
...
function CreateLamp(train_stop)
  local direction_offsets = {
    [defines.direction.north] = { x = -1, y = 1 },
    [defines.direction.east] = { x = -2, y = -1 },
    [defines.direction.south] = { x = 0, y = 1 },
    [defines.direction.west] = { x = 1, y = 0 }
  }

  local offset = direction_offsets[train_stop.direction]
  if offset then
    position = { train_stop.position.x + offset.x, train_stop.position.y + offset.y }
  end
...
function CreateConstantCombinator(train_stop)
  local direction_offsets = {
    ...
    [defines.direction.south] = { x = -1, y = 1 },
    [defines.direction.west] = { x = 1, y = -1 }
  }
...
function CreateTrainSchedule(surface, order)
  ...
  MTN_Log(LEVEL.INFO,
    ...
    order.train.id ..
    " to deliver " ..
    to_slash_notation(order.resource) ..
    " from " ..
    storage.MTL.surfaces[surface.index].stops[order.provider].train_stop.backer_name ..
    " to " .. storage.MTL.surfaces[surface.index].stops[order.requester].train_stop.backer_name)
end
